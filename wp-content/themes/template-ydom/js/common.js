jQuery(document).ready(function($) {
	...
	setTimeout(function() {
		$('.ipoteka-page .main-scaner-wrap, .ipoteka-page .advanced-search, .ipoteka-page .searchBanks').css({
			'visibility':'visible',
			'opacity':'1',
		});
	}, 400);
...
//         }))
	...
	if($('.nav-tabs li').eq(0).hasClass('active')){
		...
		$('#rev-desc').css('opacity', '0');
	}else if($('.nav-tabs li').eq(1).hasClass('active')){
		priceMinMax(1000);
		$("label[for='withRealtors']").hide();

		$('#rev-desc').css('opacity', '1');
	}
	...
	$(".wrap-term .ui-spinner-button").on('click', function(){
		var value = $("#amount").val();
		$(".slider-range").slider("value",value);
...
var funcSearchBanks = function(){
	...
	function priceMinMaxBankPrice($step){
		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-up').off();
		$('.wrap-bankPrice span.ui-spinner').eq(1).find('.ui-spinner-up').off();
		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-down').off();
		$('.wrap-bankPrice span.ui-spinner').eq(1).find('.ui-spinner-down').off();

		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-up').mousedown(function(){
			return false;
		});
		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-down').mousedown(function(){
			return false;
		});
		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-up').on('click', function(){
			var $val = $('.bankPrice.ui-spinner-input').val();
			$val = $val.replace(/ /gi, '');
			if($val){
				var $valNew = parseInt($val)+$step;
			}else{
			...
			$('.bankPrice.ui-spinner-input').val(addCommas($valNew));
		});
		$('.wrap-bankPrice span.ui-spinner').eq(0).find('.ui-spinner-down').on('click', function(){
			var $val = $('.bankPrice.ui-spinner-input').val();
			$val = $val.replace(/ /gi, '');
			if($val>0){
				var $valNew = parseInt($val)-$step;
			...
			if($valNew<=0){
				$('.bankPrice.ui-spinner-input').val(0);
			}else{
				$('.bankPrice.ui-spinner-input').val(addCommas($valNew));
			}
		});

		$('.wrap-bankPrice span.ui-spinner').eq(1).find('.ui-spinner-up').mousedown(function(){
			return false;
		});
		$('.wrap-bankPrice span.ui-spinner').eq(1).find('.ui-spinner-down').mousedown(function(){
			return false;
		});
	}
	function priceMinMaxFirstPay($step){
		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-up').off();
		$('.wrap-firstPay span.ui-spinner').eq(1).find('.ui-spinner-up').off();
		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-down').off();
		$('.wrap-firstPay span.ui-spinner').eq(1).find('.ui-spinner-down').off();

		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-up').mousedown(function(){
			return false;
		});
		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-down').mousedown(function(){
			return false;
		});
		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-up').on('click', function(){
			var $val = $('.firstPay.ui-spinner-input').val();
			$val = $val.replace(/ /gi, '');
			if($val){
				var $valNew = parseInt($val)+$step;
			}else{
			...
			$('.firstPay.ui-spinner-input').val(addCommas($valNew));
		});
		$('.wrap-firstPay span.ui-spinner').eq(0).find('.ui-spinner-down').on('click', function(){
			var $val = $('.firstPay.ui-spinner-input').val();
			$val = $val.replace(/ /gi, '');
			if($val>0){
				var $valNew = parseInt($val)-$step;
			...
			if($valNew<=0){
				$('.firstPay.ui-spinner-input').val(0);
			}else{
				$('.firstPay.ui-spinner-input').val(addCommas($valNew));
			}
		});

		$('.wrap-firstPay span.ui-spinner').eq(1).find('.ui-spinner-up').mousedown(function(){
			return false;
		});
		$('.wrap-firstPay span.ui-spinner').eq(1).find('.ui-spinner-down').mousedown(function(){
			return false;
		});
	}

	$('#buy .searchBanks').click(function(){
		Cookies.set('scrollPage', 'true');
	});
	if(Cookies.get('scrollPage') == 'true'){
		$("a.scrollRes").click();
	}
	$('.ipoteka-ico').on('click', function(){
		Cookies.set('priceIpoteka', $(this).parent('.ownership-item-price').text().replace(/[^0-9]/gim,''));
	});
	if($('.bankPrice').length && Cookies.get('priceIpoteka')){
		$('.bankPrice').val(addCommas(Cookies.get('priceIpoteka')));
		$('button.searchBanks').click();
		Cookies.set('priceIpoteka', '');
	}

	$('.bankPrice').on('input', function(){
		var $val = $(this).val().replace(/[^0-9]/gim,'');
		$val = $val.replace(/ /gi, '');
		$(this).val(addCommas($val));
	});
	$('.firstPay').on('input', function(){
		var $val = $(this).val().replace(/[^0-9]/gim,'');
		$val = $val.replace(/ /gi, '');
		$(this).val(addCommas($val));
	});
	priceMinMaxBankPrice(100000);
	priceMinMaxFirstPay(100000);
}
...
var ajaxFunc = function() {
	var nextpage = parseInt($('.show-more').data('curpage')) + 1;
